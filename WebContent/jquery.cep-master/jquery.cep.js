/**
 * jQuery CEP plugin v0.2 https://github.com/giovanneafonso/jquery.cep
 */

// TODO: Verificar se é necessário mudar para JSONP
// por causa do cross domain do Internet Explorer
// Exemplo aqui:
// http://www.eliezer.com.br/post/busca-de-cep-no-seu-form-apenas-com-javascript/
(function($) {
	/**
	 * Return only Numbers with max-length = 8
	 * 
	 * @param number|string
	 *            str Any number or string
				...
				if (cepNumber.length === 8) {
					...
					$.get(
							...
							function(responseCEP) {
								...
								var response = settings.ajax
										.responseParse(responseCEP);
								...
								.each(
										function() {
											...
											var settings = {
												autofill : true,
												autofill_attr : self
														.attr("cep-att") != null ? self
														.attr("cep-att")
														: "data-cep",
												ajax : {
													url : self
															.attr("cep-ajax-url") != null ? self
															.attr("cep-ajax-url")
															: "http://cep.republicavirtual.com.br/web_cep.php",
													requestParse : self
															.attr("cep-ajax-requestParse") != null ? function(
															request) {
														return window[self
																.attr("cep-ajax-requestParse")]
																(request);
													}
															: function(request) {
																return request;
													...
													responseParse : self
															.attr("cep-ajax-responseParse") != null ? function(
															response) {
														return window[self
																.attr("cep-ajax-responseParse")]
																(response);
													}
															: function(response) {
																return response;
												...
												init : self.attr("cep-init") != null ? function(
														cepElement) {
													window[self
															.attr("cep-init")]
															(cepElement);
												...
												done : self.attr("cep-done") != null ? function(
														cepElement, responseCEP) {
													window[self
															.attr("cep-done")](
															cepElement,
															responseCEP);
